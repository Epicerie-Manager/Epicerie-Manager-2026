...
export default function CollabPinPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const selectedName = useMemo(() => searchParams.get("name") ?? "", [searchParams]);
  ...
  return (
    <CollabPage>
      <Link href="/collab/login" style={{ color: collabTheme.accent, textDecoration: "none", fontSize: 14, fontWeight: 700 }}>← Retour</Link>
      <div style={{ height: 10 }} />
      ...
      <SectionCard style={{ padding: "22px 18px" }}>
        ...
        <div style={{ marginTop: 16, textAlign: "center", fontSize: 12, color: collabTheme.muted }}>
          Code oublié ? Contacter la responsable.
        </div>
      </SectionCard>
    </CollabPage>
  );
