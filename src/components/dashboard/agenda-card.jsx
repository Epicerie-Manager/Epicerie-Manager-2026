...
function EventIcon({ type, color, size = 13 }) {
...
function isPastEvent(event, currentHour) {
  return event.startHour < currentHour - 1;
}

function isCurrentEvent(event, currentHour) {
  return event.startHour >= currentHour - 1 && event.startHour <= currentHour;
}

function getVisibleEvents(events, currentHour, showAll) {
  if (showAll) return events;

  const firstRelevantIndex = events.findIndex((event) => event.startHour >= currentHour - 1);
  const compactStartIndex =
    firstRelevantIndex >= 0
      ? firstRelevantIndex
      : Math.max(events.length - 3, 0);

  return events.slice(compactStartIndex, compactStartIndex + 3);
}

function AgendaConnection({ connected }) {
  if (connected) {
    return (
...
function EventRow({ event, currentHour }) {
  const isPast = isPastEvent(event, currentHour);
  const isCurrent = isCurrentEvent(event, currentHour);
  ...
  return (
    <div
      style={{
        ...
        background: isCurrent ? `${event.color}08` : "#fbfcfd",
        border: `1px solid ${isCurrent ? `${event.color}25` : "#dbe3eb"}`,
        borderLeft: `3px solid ${event.color}${isPast ? "40" : ""}`,
    ...
    >
      <div
        style={{
          ...
          height: 28,
          borderRadius: 8,
          background: isCurrent ? `${event.color}15` : `${event.color}08`,
          display: "flex",
          alignItems: "center",
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div
          style={{
            ...
            color: isCurrent ? event.color : "#1e293b",
            lineHeight: 1.2,
          }}
        >
          {event.title}
        </div>
        <div style={{ fontSize: 11, color: "#617286", marginTop: 1 }}>{event.heure}</div>
      </div>
...
export default function AgendaCard({ events = MOCK_EVENTS, calendarUrl = "https://calendar.google.com" }) {
  ...
  const visibleEvents = getVisibleEvents(displayedEvents, currentHour, showAll);
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
        ...
        <AgendaConnection connected={connected} />
      </div>
      ...
      <div style={{ display: "grid", gap: 6 }}>
        {loading ? (
          ...
          <EventRow key={event.id} event={event} currentHour={currentHour} />
        ))}
      </div>
