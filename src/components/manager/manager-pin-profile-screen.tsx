...
type ManagerPinProfileScreenProps = {
  profileSlug: string;
};

export function ManagerPinProfileScreen({ profileSlug }: ManagerPinProfileScreenProps) {
  ...
  const selectedProfile = useMemo(
    () => profiles.find((profile) => profile.slug === normalizedSlug) ?? null,
    [normalizedSlug, profiles],
  );
  ...
  const handleDigit = (digit: string) => {
    ...
    setBusy(true);
    void signInManagerMobile(normalizedSlug, nextPin)
      .then(() => {
